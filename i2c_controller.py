...
class I2CWriteController(I2CController):
    ...
    def __init__(self, i2cDevice, register, pin):
        ...
        self.stateController = state.StateController()
        self.pin = pin

    def set_enabled(self):
        value = self.bus.read_byte(self.register) & ~(1 << self.pin)
        self.bus.write_byte(self.register, value)
        self.stateController.save_state(self.i2cDevice, self.register, self.pin, "ON")

    def set_disabled(self):
        value = self.bus.read_byte(self.register) | (1 << self.pin)
        self.bus.write_byte(self.register, value)
        self.stateController.save_state(self.i2cDevice, self.register, self.pin, "OFF")

    def trigger_value(self):
        value = self.bus.read_byte(self.register) ^ (1 << self.pin)
        self.bus.write_byte(self.register, value)
        result = value & (1 << self.pin)
        if result == False:
            self.stateController.save_state(self.i2cDevice, self.register, self.pin, "ON")
        else:
            self.stateController.save_state(self.i2cDevice, self.register, self.pin, "OFF")
